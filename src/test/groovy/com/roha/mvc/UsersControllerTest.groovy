...
class UsersControllerTest extends Specification {
    ...
    def "should crud users"() {
        given: "an user"
        UserDTO userDTO = new UserDTO(new User(name: "name", email: "a@a.nl", password: "password", phone:"0657334703"))
...
//        when:"updating an existinguser"
        ...
        response = mockMvc.perform(get("/users")
                ...
                .andReturn().response
        then:
        response.contentAsString == "[{\"id\":${result.id},\"name\":\"name\",\"email\":\"a@a.nl\",\"password\":\"password\",\"phone\":\"0657334703\"}]"
        response.status == 200
    }
